...
| きろく | `#page-log` | 「これに決めた」で保存された献立履歴（直近30件）、データのバックアップ（JSONエクスポート／インポート、対象キーは `BK_KEYS`） |

このほか、初回起動時のみ表示されるオンボーディング全画面（`#onboard`、3スライド）、トースト通知（`#toast`）、ボトムシート2種（`#profile` 目標カロリー計算、`#shop` 買い物リスト。共通クラス `.sheetwrap`）がある。
