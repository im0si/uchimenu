...
| めにゅー | `#page-menu` | レパートリー統計、定番からのタップ追加（`#quickList`）、自作料理の登録フォーム（写真・カテゴリ・kcal）、登録済み一覧（お気に入り⭐・削除）、にがて食材の除外設定（`#ngIn`／`#ngBtn`／`#ngListBox`、タップで解除） |
...
- `poolOf(cat)` … カテゴリ別の抽選プール。`source==="mine"` なら自分の登録料理を優先（2品未満なら定番で補完）。最後に `isNG()` でにがて食材（`ngList`）を含む料理を除外する（全滅する場合は除外しない）
- `isNG(d)` / `renderNg()` … にがて食材の判定（料理名または `ING` の材料名に部分一致）と、除外設定UIの描画。単語は12文字まで・重複登録不可・タップで解除
...
| `um_ng` | `ngList` | にがて食材の配列（文字列。抽選プールから部分一致で除外） |
